...
const consoleFormat = winston.format.combine(winston.format.colorize({ all: true }), winston.format.printf((info) => `${info.timestamp} ${info.level}: ${info.message}`));
...
export const logQueue = {
    start: (queueName) => logger.info(`Queue '${queueName}' started processing`),
    complete: (queueName) => logger.info(`Queue '${queueName}' completed processing`),
    error: (queueName, error) => logger.error(`Queue '${queueName}' error: ${error.message}`, { error }),
    jobStart: (queueName, jobId) => logger.debug(`Queue '${queueName}' started job ${jobId}`),
